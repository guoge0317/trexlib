...
def __load():
    ...
    if os.getenv("TREX_EXT_LIBS"):
        ...
        # move to default ./external_libs
        ext_libs_path = os.path.normpath(
            os.path.join(
                CURRENT_PATH,
                "external_libs",
            )
    ...
    if not os.path.exists(ext_libs_path):
        raise Exception(
            "Could not determine path of external_libs, try setting TREX_EXT_LIBS variable"
        )
...
def __import_ext_libs(ext_libs, ext_libs_path):
    ...
    for p in ext_libs:
        ...
        if not os.path.exists(full_path):
            ...
            err_msg += (
                "Please provide the correct path using TREX_EXT_LIBS variable\n\n"
            )
        ...
        if full_path not in sys.path:
            ...
            for m_name, module in dict(sys.modules).items():
                if hasattr(module, "__path__"):
                    ...
                    p_name = p["name"]
                    if m_name == p_name or m_name.startswith(
                        p_name + "."
                    ):  # lib with same name
                        first_item = next((p for p in m_path), None)
                        if not first_item or not first_item.startswith(
                            full_path
                        ):  # not our path
